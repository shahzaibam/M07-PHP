    ...
    if (isset($_POST['discount'])) {
        ...
        if ($discount) {
            ...
            //     $info['menu'] = $_POST['menu'];
                ...
                $toppings = array_keys($menuOptions);

                $_SESSION["selected_menu"] = [];

                $info["menu"] = "menu";

                if ($selected_toppings) {
                    foreach ($selected_toppings as $prod) {
                        $posicion = array_search($prod, array_keys($menuOptions));

                        $cantidadOferta = filter_input(INPUT_POST, "quantity$posicion", FILTER_VALIDATE_INT);

                        $info["quantity$posicion"] = $cantidadOferta;

                        if (in_array($prod, $toppings)) {
                            $_SESSION["selected_menu"][] = $prod;
                        }
                    }
                } else {
                    $errors["menu"] = "selecciona un menu";
                }
            // }
        }
        ...
        //     $info["quantity0"] = $_POST['quantity0'];
        // } else {
        //     $errors['quantity'] = "Por favor, selecciona al menu.";
        // }
        ...
        //     }
        // }else {
        //     $errors["menu"] = "Escoge un menu del descuento";
        // }
